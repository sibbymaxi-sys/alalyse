# ftp_dialog_gateview.py
import tkinter as tk
from tkinter import ttk, messagebox, simpledialog
import config_manager as cfg

class FTPDialogGateView(tk.Toplevel):
    def __init__(self, parent):
        super().__init__(parent)
        self.title("FTP-Profile für GateView verwalten")
    ...
    # ... (Alle Methoden sind identisch zum mv3d Dialog, aber nutzen den config key 'ftp_profiles_gateview')
    def _setup_widgets(self):
        ...
        self.profile_selector = ttk.Combobox(profile_frame, state="readonly"); self.profile_selector.grid(row=0, column=1, sticky="ew", padx=5); self.profile_selector.bind("<<ComboboxSelected>>", self._on_profile_select)
        ...
        settings_frame = ttk.Frame(main_frame); settings_frame.pack(fill=tk.BOTH, expand=True)
        conn_frame = ttk.LabelFrame(settings_frame, text="Verbindungseinstellungen", padding=10); conn_frame.pack(fill=tk.X, expand=True)
        ...
        path_frame = ttk.LabelFrame(settings_frame, text="Remote-Pfade der Log-Dateien", padding=10); path_frame.pack(fill=tk.X, expand=True, pady=10)
        for i, target in enumerate(self.log_targets):
            ttk.Label(path_frame, text=f"Pfad zu '{target}':").grid(row=i, column=0, sticky="w", pady=2)
    ...
    def _load_profile_list(self):
        ...
        if profile_names:
            ...
            if last_used in profile_names: self.profile_selector.set(last_used); self._load_profile_settings(last_used)
        else: self._clear_settings()
    def _create_new_profile(self):
        new_name = simpledialog.askstring("Neues Profil", "Geben Sie einen Namen für das neue GateView-Profil ein:", parent=self)
        if new_name and new_name not in self.profiles:
            self.profiles[new_name] = {}
            self.config["ftp_profiles_gateview"] = self.profiles
            cfg.save_profiles(self.config)
            self._load_profile_list(); self.profile_selector.set(new_name); self._clear_settings()
        elif new_name: messagebox.showerror("Fehler", "Ein GateView-Profil mit diesem Namen existiert bereits.", parent=self)
    ...
    def _on_save(self):
        ...
        self.profiles[profile_name] = {"host": self.host_entry.get(), "user": self.user_entry.get(), "passwd": self.pass_entry.get(), "remote_paths": remote_paths}
        self.config["ftp_profiles_gateview"] = self.profiles
        cfg.save_profiles(self.config); messagebox.showinfo("Gespeichert", f"Einstellungen für Profil '{profile_name}' wurden gespeichert.", parent=self)
    def _on_download(self):
        ...
        self.config["ftp_profiles_gateview"] = self.profiles
        cfg.save_profiles(self.config)
        self.result = self.profiles[profile_name]; self.destroy()
    def _on_profile_select(self, event=None): self._load_profile_settings(self.profile_selector.get())
    def _load_profile_settings(self, profile_name):
        ...
        for target, entry in self.path_entries.items():
            entry.delete(0, tk.END); entry.insert(0, remote_paths.get(target, f"/remote/path/to/{target}"))
