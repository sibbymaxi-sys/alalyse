...
class GateViewCasefileWindow(tk.Toplevel):
    def __init__(self, parent, selected_row, full_df):
    ...
    def _analyze_and_build(self):
        ...
        if pd.notna(incident_time):
            ...
            incident_df = self.full_df[(self.full_df['Timestamp'].notna()) & (self.full_df['Timestamp'] >= start_time) & (self.full_df['Timestamp'] <= end_time)]
            time_str = incident_time.strftime('%Y-%m-%d %H:%M:%S')
        else:
            incident_df = self.full_df.loc[[self.selected_row.name]]
            time_str = "N/A (konnte nicht aus Log extrahiert werden)"
        ...
        ttk.Label(header_frame, text=time_str).grid(row=0, column=1, sticky="w", padx=5)
        ...
        log_text.tag_configure("root_cause", background="#FFD2D2", font=("Courier New", 9, "bold"))
        log_text.tag_configure("error_word", foreground="orange")
